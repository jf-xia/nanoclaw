...
Browser automation for X interactions triggered through NanoClaw chats.
...
1. **NanoClaw is installed and running** - at least one channel is connected and the service is active
...
npx dotenv -e .env -- npx tsx .copilot/skills/x-integration/scripts/setup.ts
# Verify: data/x-auth.json should exist after successful login

# 2. Rebuild container to include skill
./container/build.sh
# Verify: Output shows "COPY .copilot/skills/x-integration/agent.ts"
...
```typescript
import { handleXIpc } from '../.copilot/skills/x-integration/host.js';
```
...
```typescript
// @ts-ignore - Loaded from .copilot/skills/x-integration/ during local runner build
...
```bash
echo '{"content":"Test tweet - please ignore"}' | npx dotenv -e .env -- npx tsx .copilot/skills/x-integration/scripts/post.ts
```

### Test Like

```bash
echo '{"tweetUrl":"https://x.com/user/status/123"}' | npx dotenv -e .env -- npx tsx .copilot/skills/x-integration/scripts/like.ts
```
...
echo '{"content":"Test"}' | npx tsx .copilot/skills/x-integration/scripts/post.ts
```
