...
Adds a `/compact` session command that compacts conversation history to fight context rot in long-running sessions. Uses the GitHub Copilot SDK's built-in `/compact` slash command — no synthetic system prompts.
