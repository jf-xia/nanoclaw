...
**"Credential proxy upstream error" in logs:** Check that `.env` has a valid `ANTHROPIC_API_KEY` or `GITHUB_TOKEN`. Verify the API is reachable: `curl -s https://api.anthropic.com/v1/messages -H "x-api-key: test" | head`.
...
6. Remove `ANTHROPIC_API_KEY` / `GITHUB_TOKEN` from `.env`
