...
- **Non-blocking Design** - Uses NanoClaw scheduler for result polling (no runner blocking)
...
3. NanoClaw already rebuilt with the local agent runner
...
Add `PARALLEL_API_KEY` to allowed environment variables in `src/agent-runtime.ts`:

Find the line:
```typescript
const allowedVars = ['GITHUB_TOKEN', 'ANTHROPIC_API_KEY'];
```

Replace with:
```typescript
const allowedVars = ['GITHUB_TOKEN', 'ANTHROPIC_API_KEY', 'PARALLEL_API_KEY'];
```
...
Build the local agent runner with updated code:

```bash
./container/build.sh
```

Verify the build by checking the compiled entry exists:
```bash
...
- Verify agent-runtime.ts includes PARALLEL_API_KEY in allowedVars
- Check agent-runner logs for "Parallel AI MCP servers configured" message

**Task polling not working:**
- Verify scheduled task was created: `cat data/scheduled_tasks.json`
...
2. Revert changes to agent-runtime.ts and agent-runner/src/index.ts
