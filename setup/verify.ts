...
import { MOUNT_ALLOWLIST_PATH, STORE_DIR } from '../src/config.js';
...
export async function run(_args: string[]): Promise<void> {
  ...
  const agentRunnerDist = path.join(projectRoot, 'container', 'agent-runner', 'dist', 'index.js');
  ...
  if (fs.existsSync(envFile)) {
    ...
    if (/^(export\s+)?(GITHUB_TOKEN|ANTHROPIC_API_KEY|ANTHROPIC_AUTH_TOKEN|ONECLI_URL)=/m.test(envContent)) {
