...
その後、`/setup`を実行します。Claude Codeが依存関係、認証、ローカル agent-runner のビルド、サービス設定を処理します。
...
- **グループごとの分離コンテキスト** - 各グループは独自の`CLAUDE.md`メモリ、作業ディレクトリ、Copilot セッション状態を持ちます。
...
```
チャネル --> SQLite --> ポーリングループ --> ローカル Agent Runner（Claude Agent SDK） --> レスポンス
```
