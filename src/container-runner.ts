...
import { getAgentRunnerPath } from './container-runtime.js';
...
function buildAgentEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    NANOCLAW_SESSION_DIR: groupSessionsDir,
...
export async function runContainerAgent(
  ...
  onProcess: (proc: ChildProcess, agentId: string) => void,
  ...
  logger.debug(
    {
      ...
      agentId,
      agentRunnerPath,
      ipcDir: agentEnv.NANOCLAW_IPC_DIR,
      groupDir: agentEnv.NANOCLAW_GROUP_DIR,
    },
    'Agent configuration',
  );
  ...
  return new Promise((resolve) => {
    ...
    agent.on('close', (code) => {
      ...
      if (timedOut) {
        ...
        if (hadStreamingOutput) {
          logger.info(
            { group: group.name, agentId, duration, code },
    ...
    agent.on('error', (err) => {
      clearTimeout(timeout);
      logger.error(
        { group: group.name, agentId, error: err },
        'Agent spawn error',
      );
      resolve({
        status: 'error',
