...
beforeEach(() => {
  ...
  groups = {
    'main@chat.local': MAIN_GROUP,
    'other@chat.local': OTHER_GROUP,
    'third@chat.local': THIRD_GROUP,
  };

  // Populate storage as well
  setRegisteredGroup('main@chat.local', MAIN_GROUP);
  setRegisteredGroup('other@chat.local', OTHER_GROUP);
  setRegisteredGroup('third@chat.local', THIRD_GROUP);
...
describe('schedule_task authorization', () => {
  it('main group can schedule for another group', async () => {
    ...
    // Verify task was created in storage for the other group
...
describe('pause_task authorization', () => {
  beforeEach(() => {
    createTask({
      id: 'task-main',
      group_folder: 'main-control',
      chat_jid: 'main@chat.local',
      prompt: 'main task',
      schedule_type: 'once',
      schedule_value: '2025-06-01T00:00:00',
...
describe('register_group authorization', () => {
  it('non-main group cannot register a group', async () => {
    ...
    expect(groups['new@chat.local']).toBeUndefined();
  });
...
describe('IPC message authorization', () => {
  ...
  it('main group can send to any group', () => {
    expect(
      isMessageAuthorized('main-control', true, 'other@chat.local', groups),
    ).toBe(true);
    expect(
      isMessageAuthorized('main-control', true, 'third@chat.local', groups),
    ).toBe(true);
  });

  it('non-main group can send to its own chat', () => {
    expect(
      isMessageAuthorized('other-group', false, 'other@chat.local', groups),
    ).toBe(true);
  });

  it('non-main group cannot send to another groups chat', () => {
    expect(isMessageAuthorized('other-group', false, 'main@chat.local', groups)).toBe(
      false,
    );
    expect(
      isMessageAuthorized('other-group', false, 'third@chat.local', groups),
    ).toBe(false);
  });

  it('non-main group cannot send to unregistered JID', () => {
    expect(
      isMessageAuthorized('other-group', false, 'unknown@chat.local', groups),
    ).toBe(false);
  });
  ...
  it('main group can send to unregistered JID', () => {
    ...
    expect(
      isMessageAuthorized('main-control', true, 'unknown@chat.local', groups),
    ).toBe(true);
  });
});
...
describe('register_group success', () => {
  it('main group can register a new group', async () => {
    ...
    // Verify group was registered in storage
    const group = getRegisteredGroup('new@chat.local');
  ...
  it('register_group rejects request with missing fields', async () => {
    ...
    expect(getRegisteredGroup('partial@chat.local')).toBeUndefined();
  });
});
