...
function migrateLegacyStateToJson(database: Database.Database): void {
  if (!hasRouterStateStore()) {
    try {
      const rows = database.prepare('SELECT key, value FROM router_state').all() as Array<{
        key: string;
        value: string;
      }>;
      if (rows.length > 0) {
      ...
      /* router_state table does not exist or is unreadable */
    }
  }
  ...
  if (!hasSessionsStore()) {
    try {
      ...
      /* sessions table does not exist or is unreadable */
    }
  }
  ...
  if (!hasRegisteredGroupsStore()) {
    try {
      ...
      /* registered_groups table does not exist or is unreadable */
    }
  }
}
