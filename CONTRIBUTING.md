...
NanoClaw uses [Copilot CLI skills](https://docs.github.com/copilot/how-tos/copilot-cli) — markdown files with optional supporting files that teach Copilot how to do something. There are four types of skills in NanoClaw, each serving a different purpose.
...
**Location:** `.copilot/skills/` on `main` (instructions only), code on `skill/*` branch
...
3. Copilot walks through interactive setup (env vars, bot creation, etc.)
...
3. Add a SKILL.md in `.copilot/skills/<name>/` with setup instructions — step 1 should be merging the branch
...
**Location:** `.copilot/skills/<name>/` with supporting files
...
- Use `${COPILOT_WORKFLOW_DIR}` to reference files in the skill directory
...
**Location:** `.copilot/skills/` on `main`
...
Skills that run inside the agent container, not on the host. These teach the container agent how to use tools, format output, or perform tasks. They are synced into each group's `.copilot/skills/` directory when a container starts.
...
All skills use the [Copilot CLI skills standard](https://docs.github.com/copilot/how-tos/copilot-cli):
...
- `description`: required — Copilot uses this to decide when to invoke the skill
- Put code in separate files, not inline in the markdown
- See the [skills standard](https://docs.github.com/copilot/how-tos/copilot-cli) for all available frontmatter fields
