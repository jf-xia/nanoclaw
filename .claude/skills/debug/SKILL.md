...
```bash
grep -E 'CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|ANTHROPIC_AUTH_TOKEN|ONECLI_URL' .env
```
...
[ -f .env ] && (grep -q "CLAUDE_CODE_OAUTH_TOKEN=sk-" .env || grep -q "ANTHROPIC_API_KEY=sk-" .env || grep -q "ANTHROPIC_AUTH_TOKEN=" .env) && echo "OK" || echo "MISSING - add credentials to .env or configure host auth"
